...
class IzinController extends GetxController {
  ...
  Future<void> fetchIzinList({int? bulan, int? tahun}) async {
    ...
    try {
      final uri = Uri.parse('http://192.168.1.23:8000/api/izin').replace(queryParameters: {
  ...
  Future<bool> ajukanIzin({
    ...
    try {
      var uri = Uri.parse('http://192.168.1.23:8000/api/izin');
  ...
  Future<Map<String, dynamic>?> getDetailIzin(String id) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.1.23:8000/api/izin/$id'),
        headers: {
          'Authorization': 'Bearer $token',
          'Accept': 'application/json',
        },
